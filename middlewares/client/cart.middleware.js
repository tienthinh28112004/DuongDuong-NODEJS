...
module.exports.cartId = async(req,res,next) =>{
    ...
    if(!req.cookies.cartId){//kiểm tra xem trong cookie đã lưu id của giỏi hàng chưa
        ...
        cart.totalQuantity = cart.products.reduce((sum,item) => sum + item.quantity, 0);//tính tổng số lượng sản phẩm

        res.locals.miniCart = cart;//đưa biến minicart thành biến toàn cục
    }
